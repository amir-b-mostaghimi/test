...
def train_agent(env_name='CartPole-v1', episodes=1000, target_update=10):
    ...
    for episode in range(episodes):
        ...
        if total_reward > max_reward:
            ...
            if total_reward > 100:  # Only record if performance is good
                ...
                record_env = RecordVideo(
                    ...
                    name_prefix=f"cart_pole-best_episode_{episode}_reward_{int(total_reward)}"
                )
                test_reward = evaluate_agent(agent, record_env)
                record_env.close()
                print(f"Recorded episode finished with reward {test_reward}")
